...
def get_bad_trials(participants):
    ...
    for i in range(len(participants)):
        ...
        if len(p_df) == 0:  # if this participant's trials are all good (i.e. no bad trials)
            ...
            ybad.append(get_ybad_from_cel_obs(participants, i, bad_trial_df))   # Retrieve bad trials based on the cell and obs columns
...
def get_ybad_from_cel_obs(participants, i, df):
    bad_trials = []
    orig_trial_df = pd.read_csv("%s%s_trial_cell_obs.csv" % (db_filepath, participants[i]))
    for index, row in df.iterrows():
        # get the trial index by searching for matched combination of cell, obs and participant value
        bad_trials += orig_trial_df[(orig_trial_df['cell'] == row['Cell']) & (orig_trial_df['obs'] == row['Observation']) & (int(participants[i]) == row['Ps'])].trial_index.values.tolist()
    return bad_trials

def get_left_trial_each_word(participant):
    orig_word_count_df = pd.read_csv("%s%s_trial_cell_obs.csv" % (db_filepath, participant))
