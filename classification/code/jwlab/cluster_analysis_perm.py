import pandas as pd
import numpy as np
from jwlab.ml_prep_perm import  average_trials_and_participants, no_average, average_trials
...
def slide_df(df, length_per_window):
    total_num_windows = int(1200 / length_per_window)
    num_windows = int(1000 / length_per_window) # = 100
    windows_list = []
    for i in range(20, 0, -1):
        windows_list.append(df[(df.Time < -(i-1) * length_per_window) & (df.Time >= -i * length_per_window)])
    for i in range(num_windows):
        windows_list.append(df[(df.Time < ((i+1) * length_per_window)) & (df.Time >= i * length_per_window)])
    assert len(windows_list) == total_num_windows
    return windows_list
...
def prep_cluster_analysis_internal_permutation(df, ys, participants, downsample_num=1000, length_per_window=100, useRandomizedLabel=False):
...
def prep_cluster_analysis_internal(df, ys, participants, downsample_num=1000, averaging="average_trials_and_participants", length_per_window=10, useRandomizedLabel=False):
    # for the ml segment we only want post-onset data, ie. sections of each epoch where t>=0
    df = df[df.Time >= 0]
...
def cross_validaton(num_iterations, num_sliding_windows, num_folds, X, y):
    ...
    for i in range(num_sliding_windows):
        ...
        scoreMean.append(round(np.mean(results[i]), 2))
        stdev.append(round(np.std(results[i]), 2))
