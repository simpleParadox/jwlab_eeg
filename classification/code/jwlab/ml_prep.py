...
def prep_ml_internal(df, ys, participants, downsample_num=1000, averaging="average_trials"):
    ...
    X_list = [[0 for i in range(int((1000-sliding_window_time_length[j])/100))] for j in range(len(sliding_window_time_length))]
    y_list = [[0 for i in range(int((1000-sliding_window_time_length[j])/100))] for j in range(len(sliding_window_time_length))]
    p_list = [[0 for i in range(int((1000-sliding_window_time_length[j])/100))] for j in range(len(sliding_window_time_length))]
    w_list = [[0 for i in range(int((1000-sliding_window_time_length[j])/100))] for j in range(len(sliding_window_time_length))]
    df_list = [[0 for i in range(int((1000-sliding_window_time_length[j])/100))] for j in range(len(sliding_window_time_length))]
    for each_timeLength in range(len(sw_list_for_all_time_length)):
        for each_df in range(len(sw_list_for_all_time_length[each_timeLength])):
            ...
            X = df.values
            X = np.reshape(X, (sliding_window_time_length[each_timeLength], 60, -1))
            ...
            trial_count = []
            bad_trial_count = []
            for each_ps in range(len(ys_curr)):
                for bad_trial in range(len(ybad[each_ps])):
                    ...
                    ys_curr[each_ps][ybad[each_ps][bad_trial]-1] = -1

                # count the total number of trials for each participant
                trial_count += [len(ys_curr[each_ps])]
    ...
    return X_list, y_list, p_list, w_list, df_list
