...
averaging_1 = "no_average_labels"
averaging_2 = "no_average_labels"
...
for m in range(matrix_build):
    ...
    X_1, y_1, good_trial_count_1, num_win_1 = prep_ml(age_group_1, useRandomizedLabel, averaging_1, sliding_window_config, downsample_num=1000)
    X_2, y_2, good_trial_count_2, num_win_2 = prep_ml(age_group_2, useRandomizedLabel, averaging_2, sliding_window_config, downsample_num=1000)

    num_indices = len(X_1[0][0])
    #split say 15 times?
    ...
    for f in range(fold):
        ...
        # assert len(X_1[0]) == len(X_2[0])
        num_sliding_windows = len(X_1[0])
        ...
        for i in range(int(num_sliding_windows)):
            ...
            # Processing the training set here.
            if 'level_0' in X_1[0][i].columns:
                X_1[0][i] = X_1[0][i].drop(columns = ['level_0'], axis = 1)
            X_1[0][i] = X_1[0][i].reset_index()
            if 'level_0' in X_2[0][i].columns:
                X_2[0][i] = X_2[0][i].drop(columns = ['level_0'], axis = 1)
            X_2[0][i] = X_2[0][i].reset_index()

            # #create new df with these indices and removing from orig
            df_test.append(X_2[0][i])
            df_train.append(X_1[0][i])
            assert(len(df_train[i]) + len(df_test[i]) == (len(X_1[0][i]) + len(X_2[0][i])))
        ...
        if len(num_win_1) > 1:
            ...
            for i in range(num_win_1[0]):
                 for j in range(num_win_2[0]):
...
time_table.to_csv('Train 12m raw test 9m avg_trials_and_ps 50ms_10msSteps_verification3.csv')
