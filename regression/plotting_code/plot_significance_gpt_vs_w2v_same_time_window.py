...
def plot_image_same_time_with_cluster(results_1_dict,
    ...
    **kwargs : dict
    ...
    # Find the onset index (where time is closest to or equal to 0)
    times = np.array(time_window)
    onset_idx = np.where(times >= 0)[0][0]  # First index where time is >= 0
    print(f"Onset occurs at index {onset_idx}, time={times[onset_idx]}ms")
    
    # Filter windows to only include those after onset
    post_onset_windows = [w for i, w in enumerate(windows) if i >= onset_idx]
    n_times = len(post_onset_windows)
    print(f"Using {n_times} time windows after onset")
    
    # Check that we have sample data for first window to determine n_subjects
    if len(results_1_dict[0][post_onset_windows[0]]) == 0:
    ...
    T_obs, clusters, cluster_p_values, H0 = permutation_cluster_test(
        [data1, data2], n_permutations=n_permutations, threshold=None, tail=1, n_jobs=1, seed=kwargs.get('seed', None)
    )
    ...
    for i_c, c in enumerate(clusters):
        if cluster_p_values[i_c] <= alpha:
            # Get indices of cluster timepoints - now need to shift indices to account for onset
            print("Cluster: ", c)
            # Adjust cluster indices to match original timeline
            adjusted_cluster = [idx + onset_idx for idx in c[0]]
            cluster_inds = [adjusted_cluster[0], adjusted_cluster[-1]] 
            
            if len(cluster_inds) > 0:
                label = f'Significant (p = {cluster_p_values[i_c]:.3f})'
                labels.append(label)
                
                # All clusters will be after onset by design
                clusters_after_onset.append(adjusted_cluster)
                labels_after_onset.append(label)
                print(f"Significant cluster found: windows {times[cluster_inds[0]]} to {times[cluster_inds[-1]]}, p={cluster_p_values[i_c]:.4f}")
    ...
    if largest_cluster is not None:
        print(f"Largest cluster found: windows {times[largest_cluster[0]]} to {times[largest_cluster[-1]]}")        
        ax.axvspan(times[largest_cluster[0]], times[largest_cluster[-1]], 
...
if __name__ == '__main__':
    ...
    # """
    # The following is the code for across group results.
    # """
